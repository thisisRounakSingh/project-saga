...
export async function getSagaSession(sessionId: string): Promise<SagaSession | null> {
  // Currently we only support the vscode-demo fixture.
  // In the future this will be replaced with a real database/API call.
  const fixturePath = path.join(process.cwd(), 'fixtures', 'sessions', `${sessionId}.saga.json`);
  ...
  try {
    ...
    console.error(`Failed to load saga session fixture for id: ${sessionId}`, error);
    return null;
  }
}
