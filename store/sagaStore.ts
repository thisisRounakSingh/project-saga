...
interface SagaStoreState {
  ...
  addTrashCanFile: (file: { id: string, name: string, summary: string, actId: string }) => void;
  clearTrashCan: () => void;
...
export const useSagaStore = create<SagaStoreState>((set) => ({
  ...
  activeSearchIndex: 0,
  joystickSpeed: 8,
  chatTabs: [{ id: 'default', title: 'Main Chat', chatHistory: [], pendingChatContext: [], isArchived: false }],
  ...
  setActiveActId: (id) => set({ activeActId: id, searchQuery: '', searchResults: [], activeSearchIndex: 0, activeNarrationIndex: 0 }),
  ...
  addTrashCanFile: (file) => set((state) => { 
    if (state.trashCanFiles.find(f => f.id === file.id && f.actId === file.actId)) return state;
    return { trashCanFiles: [...state.trashCanFiles, file] };
  }),
  clearTrashCan: () => set({ trashCanFiles: [] }),
  
  createChatTab: () => set((state) => {
    const newTabId = Date.now().toString();
    return {
      chatTabs: [...state.chatTabs, { id: newTabId, title: `Chat ${state.chatTabs.length + 1}`, chatHistory: [], pendingChatContext: [], isArchived: false }],
      activeChatTabId: newTabId
    };
  }),
  switchChatTab: (id) => set({ activeChatTabId: id }),
  closeChatTab: (id) => set((state) => {
    const newTabs = state.chatTabs.map(t => t.id === id ? { ...t, isArchived: true } : t);
    const activeTabs = newTabs.filter(t => !t.isArchived);
    
    if (activeTabs.length === 0) {
      const defaultTab = { id: Date.now().toString(), title: 'Main Chat', chatHistory: [], pendingChatContext: [], isArchived: false };
      return { chatTabs: [...newTabs, defaultTab], activeChatTabId: defaultTab.id };
    }
    
    return { 
      chatTabs: newTabs, 
      activeChatTabId: state.activeChatTabId === id ? activeTabs[activeTabs.length - 1].id : state.activeChatTabId 
    };
  }),
  restoreChatTab: (id) => set((state) => ({
    chatTabs: state.chatTabs.map(t => t.id === id ? { ...t, isArchived: false } : t),
    activeChatTabId: id
  })),
  addPendingChatContext: (item) => set((state) => {
    return { chatTabs: state.chatTabs.map(tab => {
      if (tab.id === state.activeChatTabId) {
        // Prevent duplicates
        if (tab.pendingChatContext.some(ctx => ctx.text === item.text)) return tab;
        return { ...tab, pendingChatContext: [...tab.pendingChatContext, item] };
      }
      return tab;
    })};
  }),
  removePendingChatContext: (index) => set((state) => ({
    chatTabs: state.chatTabs.map(tab => 
      tab.id === state.activeChatTabId 
        ? { ...tab, pendingChatContext: tab.pendingChatContext.filter((_, i) => i !== index) } 
        : tab
    )
  })),
  clearPendingChatContext: () => set((state) => ({
    chatTabs: state.chatTabs.map(tab => 
      tab.id === state.activeChatTabId ? { ...tab, pendingChatContext: [] } : tab
    )
  })),
  pushChatMessage: (msg) => set((state) => ({
    chatTabs: state.chatTabs.map(tab => 
      tab.id === state.activeChatTabId ? { ...tab, chatHistory: [...tab.chatHistory, msg] } : tab
    )
  })),
  submitDirectQuestion: (text) => {
    ...
    setTimeout(() => {
      ...
      let aiResponseText = "I see. Let's look closely at how that impacts the architecture.";
