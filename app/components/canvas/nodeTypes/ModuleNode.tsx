...
export function ModuleNode({ data, id }: { data: ModuleNodeData; id: string }) {
  ...
  const isContextMenuAnchor = useSagaStore(state => state.contextMenuNodeId === id);
  ...
  const nextAct = activeActIndex >= 0 && activeActIndex < acts.length - 1 ? acts[activeActIndex + 1] : null;
  ...
  const getFileTypeBorder = () => {
    ...
    if (lowerPath.includes('service') || lowerPath.includes('core') || lowerPath.includes('util') || lowerName.includes('service')) {
      return 'border-red-500 dark:border-red-400'; // Business Logic
    }
    if (lowerPath.includes('component') || lowerPath.includes('page') || lowerPath.includes('ui') || lowerPath.endsWith('.tsx') || lowerPath.endsWith('.jsx')) {
      return 'border-blue-500 dark:border-blue-400'; // UI
    }
    if (lowerPath.includes('store') || lowerPath.includes('db') || lowerPath.includes('repo')) {
  ...
  const handleAskAboutThis = (e: React.MouseEvent) => {
    e.stopPropagation();
    const paths = selectedNodeIds.length > 1 
      ? selectedNodeIds.map(sid => activeAct?.modules.find(m => m.name === sid)?.path).filter(Boolean).join(', ') 
      : data.path;
  ...
  return (
    ...
    >
      ...
      {isContextMenuAnchor && (
        ...
        >
          <div>
            <h3 className="font-black text-lg tracking-tighter truncate" title={isMultipleSelected ? `${selectedNodeIds.length} files selected` : data.name}>
              {isMultipleSelected ? `${selectedNodeIds.length} files selected` : data.name}
            </h3>
            <p className="text-xs text-muted font-bold tracking-widest break-all line-clamp-2" title={isMultipleSelected ? 'Multiple files' : data.path}>
              {isMultipleSelected ? 'Multiple files' : data.path}
            </p>
          </div>

          <div className={`grid ${isMultipleSelected ? 'grid-cols-1' : 'grid-cols-2'} gap-2`}>
            {!isMultipleSelected && (
              <div className="bg-muted/10 p-2 border-2 border-border">
                <span className="block text-[8px] font-black uppercase tracking-widest text-muted">Status</span>
                <span className="text-xs font-bold capitalize">{data.status}</span>
              </div>
            )}
            <div className="bg-muted/10 p-2 border-2 border-border">
              <span className="block text-[8px] font-black uppercase tracking-widest text-muted">Lines</span>
      ...
      {data.status === 'deleted' && (
        <div 
          ...
          title={nextAct ? `This file is no longer present in Act ${nextAct.order}` : 'This file has been removed'}
        >
