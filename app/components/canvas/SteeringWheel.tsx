...
import { ZoomIn, ZoomOut, Move, Settings, Pen, EyeOff } from "lucide-react";
...
export function SteeringWheel() {
  const { setViewport, getViewport, getNodes, zoomIn, zoomOut, fitView } =
    useReactFlow();
        ...
        if (joystickPos.current.x !== 0 || joystickPos.current.y !== 0) {
          ...
          if (nodes.length > 0) {
            ...
            const dist = Math.sqrt(
              Math.pow(projectedX - centerX, 2) +
                Math.pow(projectedY - centerY, 2),
            );
            if (dist > 2500) {
          ...
          const speed =
            useSagaStore.getState().joystickSpeed * (1 / zoom) * distanceDecay;
          const maxDist = 30;
      ...
      >
        <AnimatePresence>
          {showSettings && isHovered && (
            ...
            >
              <label className="text-[10px] font-bold font-mono tracking-wider text-muted text-center">
                VELOCITY
              </label>
              <input
                type="range"
        ...
        {/* Drag Indicator */}
        <div
          className={`absolute top-2 text-muted pointer-events-none transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"}`}
        >
          <Move size={14} />
        </div>
        ...
        >
          <button
            ...
            className="pointer-events-auto p-1.5 bg-background border-[3px] border-border rounded-full hover:bg-muted/30 hover:scale-110 transition-all cursor-pointer text-muted"
          >
            {" "}
            <Settings size={14} />{" "}
          </button>
          <button
            ...
            className={`pointer-events-auto p-1.5 border-[3px] border-border rounded-full hover:scale-110 transition-all cursor-pointer ${isWhiteboardMode ? "bg-accent text-white" : "bg-background hover:bg-muted/30 text-muted"}`}
          >
            {" "}
            <Pen size={14} />{" "}
          </button>
        ...
        {/* Zoom Controls */}
        <div
          className={`absolute inset-0 rounded-full flex justify-between items-center px-4 pointer-events-none transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"}`}
        >
          <button
            onPointerDown={(e) => e.stopPropagation()}
            onClick={(e) => {
            ...
            className="pointer-events-auto p-2 bg-background border-[3px] border-border rounded-full hover:bg-muted/30 hover:scale-110 transition-all cursor-pointer"
          >
            {" "}
        ...
        {/* Fit View Control */}
        <div
          className={`absolute bottom-3 pointer-events-none transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"}`}
        >
          <button
            onPointerDown={(e) => e.stopPropagation()}
            onClick={(e) => {
            ...
            className="pointer-events-auto px-2 py-0.5 bg-background border-[3px] border-border rounded-full hover:bg-muted/30 hover:scale-110 transition-all cursor-pointer text-[10px] font-bold font-mono tracking-wider text-foreground"
          >
            {" "}
            FIT{" "}
          </button>
        ...
        <motion.div
          ...
          onDrag={(e, info) => {
            ...
            joystickPos.current = {
              x: clamp(info.offset.x, -30, 30),
              y: clamp(info.offset.y, -30, 30),
            };
            startPanning();
          }}
          onDragEnd={() => {
      ...
      {currentSession?.isViewOnly && (
        <div className="absolute z-50 bottom-27.5 left-8">
          <button
            ...
            className="px-3 py-1.5 flex items-center gap-2 text-xs font-bold uppercase tracking-widest bg-yellow-500/20 text-yellow-700 dark:text-yellow-400 border-[3px] border-yellow-500/50 shadow-[4px_4px_0_var(--color-border)] dark:shadow-[4px_4px_0_#fff] rounded-full hover:bg-yellow-500 hover:text-white transition-colors pointer-events-auto"
          >
            <EyeOff size={14} /> View-Only Mode
          </button>
        </div>
      )}
