...
export function FileInspector() {
  ...
  const nodeData = activeAct?.modules.find(m => m.name === selectedNodeIds[0]);
    ...
    <div className="flex-1 p-6 overflow-y-auto space-y-6">
      <div>
        <h2 className="text-2xl font-black tracking-tighter mb-2 break-all">
          {isMultipleSelected ? `${selectedNodeIds.length} files selected` : nodeData.name}
        </h2>
      ...
      <div className={`grid ${isMultipleSelected ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
        {!isMultipleSelected && (
          <div className={`p-4 border-[3px] border-border ${nodeData.status === 'deleted' ? 'bg-red-500/10 border-red-500 text-red-500' : 'bg-muted/10'}`}>
      ...
      <div className="bg-muted/10 p-4 border-[3px] border-border">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-muted mb-2">Auto-Generated Report</h3>
