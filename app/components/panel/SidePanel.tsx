...
import { BookOpen, MessageSquare, Layers, FileText, Maximize2, Minimize2, ArrowRightToLine, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function SidePanel() {
  ...
  const handleNextAct = () => {
    if (currentActIndex >= 0 && currentActIndex < acts.length - 1) setActiveActId(acts[currentActIndex + 1].id);
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden h-full">
        ...
        {/* Docked Icon Rail */}
        <div className={`flex flex-col items-center py-4 gap-4 w-16 shrink-0 border-border ${!isDocked ? 'border-r-[3px] bg-muted/10' : ''}`}>
          <TabButton 
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <div className="flex items-center bg-muted/10 rounded mr-2 border border-border overflow-hidden">
                    ...
                    <button 
                      ...
                      disabled={currentActIndex >= acts.length - 1 || currentActIndex === -1}
                      className="p-1 hover:bg-muted/20 hover:text-accent disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-foreground transition-colors"
                      title="Next Act"
                    >
                  ...
                  <button 
                    onClick={() => setPanelState(isMaximized ? 'expanded' : 'maximized')}
...
function TabButton({ icon, isActive, onClick, label }: { icon: React.ReactNode, isActive: boolean, onClick: () => void, label: string }) {
  return (
    <button
      onClick={onClick}
      title={label}
