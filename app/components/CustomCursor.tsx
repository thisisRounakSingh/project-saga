...
export function CustomCursor() {
  ...
  const [cursorType, setCursorType] = useState<
    "default" | "text" | "pointer" | "canvas" | "canvas-dragging"
  >("default");

  useEffect(() => {
    const checkState = () =>
      setIsNative(document.body.classList.contains("native-cursor"));
    checkState();
  ...
  return (
    <div
      ref={cursorRef}
      className="pointer-events-none fixed top-0 left-0 will-change-transform mix-blend-difference"
      style={{
        zIndex: 2147483647,
        color: "white", // mix-blend-difference against white always produces exact opposite color
      }}
    >
      {cursorType === "text" && (
        <div className="w-0.5 h-6 absolute -top-3 -left-px bg-white mix-blend-difference" />
      )}

      {cursorType === "pointer" && (
        <div
          className="w-7.5 h-7.5 rounded-full absolute -top-3.75 -left-3.75 bg-transparent transition-all duration-150 ease-out mix-blend-difference"
      ...
      {cursorType === "canvas" && (
        <div
          className="w-6 h-6 rounded-full absolute -top-3 -left-3 bg-transparent transition-all duration-150 ease-out mix-blend-difference"
          style={{ border: `3px solid white` }}
        />
      )}

      {cursorType === "canvas-dragging" && (
        <div className="w-3 h-3 rounded-full absolute -top-1.5 -left-1.5 transition-all duration-150 ease-out mix-blend-difference bg-white" />
      )}
