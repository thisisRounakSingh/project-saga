...
import { Play, Pause, SkipBack, SkipForward, Clapperboard, X, Gauge } from 'lucide-react';
...
export function PlaybackControls() {
  ...
  const togglePlay = () => {
    ...
    if (!isPlaying) {
      if (activeIndex === acts.length - 1 && state.activeNarrationIndex >= (acts[acts.length - 1].narration.length - 1)) {
      ...
      if (currentAct) {
        const nIndex = state.activeNarrationIndex < currentAct.narration.length ? state.activeNarrationIndex : 0;
  ...
  // Speed labels
  const speedLabel = narrationSpeed <= 3500 ? 'Fast' : narrationSpeed <= 5500 ? 'Normal' : 'Slow';
  ...
  useEffect(() => {
    ...
    if (isPlaying && !isTransitioningAct) {
      ...
      interval = setInterval(() => {
        ...
        const currentActIndex = acts.findIndex(a => a.id === state.activeActId);
  ...
  return (
    <div className="flex items-center bg-background border-[3px] border-border shadow-[4px_4px_0_var(--color-border)] dark:shadow-[4px_4px_0_#fff]">
      ...
      {/* Speed control */}
      <div className="flex items-center gap-2 px-3 border-r-[3px] border-border" title={`Narration Speed: ${speedLabel}`}>
        ...
        />
        <span className="text-[9px] font-black uppercase tracking-widest text-muted w-10">{speedLabel}</span>
      </div>
