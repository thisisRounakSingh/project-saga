...
export function SearchBar() {
  ...
  const setActiveSearchIndex = useSagaStore(state => state.setActiveSearchIndex);
  ...
  useEffect(() => {
    ...
    const matches = activeAct.modules
      .filter(m => m.name.toLowerCase().includes(lowerQuery) || m.summary.toLowerCase().includes(lowerQuery))
        ...
        {searchQuery && (
          ...
          >
            {searchResults.length > 0 ? (
              ...
              <span className="text-xs font-mono font-bold text-accent mr-1">0/0</span>
            )}
            
            <div className="flex flex-col">
              <button onClick={(e) => { e.stopPropagation(); handlePrev(); }} className="hover:text-accent hover:bg-muted/20 rounded p-0.5"><ChevronUp size={14}/></button>
              <button onClick={(e) => { e.stopPropagation(); handleNext(); }} className="hover:text-accent hover:bg-muted/20 rounded p-0.5"><ChevronDown size={14}/></button>
            </div>
            
            <div className="w-px h-6 bg-border mx-1" />
            
            <button onClick={(e) => { e.stopPropagation(); handleClear(); }} className="p-1 hover:text-accent hover:bg-muted/20 rounded">
              <X size={16} />
            </button>
          </motion.div>
        )}
